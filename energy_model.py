...
def plot_prediction_analysis(y, y_pred):
    fig = px.scatter(
        x=y, y=y_pred, labels={"x": "ground truth", "y": "prediction"}
    )
    fig.add_shape(
        type="line",
        line=dict(dash="dash"),
...
def xgboost_regression(
    ...
    if (
        ...
        or (grid_search and bay_opt)
        or (random_search and bay_opt)
    ):
    ...
    if random_search:
        randomized_search = RandomizedSearchCV(
            model, params, n_iter=20, scoring=scoring, cv=3, verbose=3
        )
    ...
    if grid_search:
        gridded_search = GridSearchCV(
            model, params, scoring=scoring, verbose=3
        )
    ...
    if bay_opt:
        bay_optimization = BayesianOptimization(
            xgb_evaluate,
            {
                "max_depth": (3, 7),
                "gamma": (0, 1),
        ...
        best_params = bay_optimization.res[targets.index(max(targets))][
            "params"
        ]
    ...
    if pickle:
        input(
            "press enter if you are happy with error and would like to pickle model"
        )
...
if __name__ == "__main__":
    ...
    # Plotly scatter y=x
    fig = px.scatter(
        x=y_test, y=y_preds, labels={"x": "ground truth", "y": "prediction"}
    )
    fig.add_shape(
        type="line",
        line=dict(dash="dash"),
